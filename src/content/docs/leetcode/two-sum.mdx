...
2. Iterate through `nums`. Check if the key-value store contains the difference between the current element and the target: 
    - If it does, return the current index and the index of the difference
    - If it doesn't, add the current element to the key-value store
